define(["three", "camera", "container", "geometry", "material", "noise", "renderer", "scene", "terrain"],
  function (THREE, camera, container, geometry, material, noise, renderer, scene, Terrain) {
    var app = {
      ...
      animate: function () {
        callbackId = window.requestAnimationFrame(app.animate);
        ...
        //        camera.position.x = 450 * Math.cos(time / 3) + app.mouse.x;
        //        camera.position.y = 250 * Math.sin(time / 4) + app.mouse.y + 500;
        camera.position.x = 450 * Math.cos(time / 3);
        camera.position.y = 250 * Math.sin(time / 4) + 500;
